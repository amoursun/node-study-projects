// https://github.com/luyuqi/node-js-spider/blob/master/index.js
const cheerio = require('cheerio');
const fs = require('fs');
const qs = require('qs');
const axios = require('axios');
const jsSHA = require('jssha');
const encrypt = require('./assets/encrypt');
const url = {
...
function putCaptcha(set_cookie) {
    return axios({
        ...
        .then(response => {
            ...
            fs.writeFile('./assets/base64.txt', response.data.img_base64, (err) => {
                if (err) throw err;
                console.log('文件已被保存');
            });
            fs.writeFile('./assets/base64.png', dataBuffer, (err) => {
                if (err) throw err;
                console.log('照片已被保存');
            });
