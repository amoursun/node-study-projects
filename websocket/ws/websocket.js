...
const encodeMessage = function(opcode, payload, isFinal = true) {
  let buf;
  const b1 = (isFinal ? 0x80 : 0x00) | opcode;
  let b2;
 ...
 * */
  ...
  _processBuffer() {
    ...
    const FIN = byte1 & 0x80; // 如果为0x80，则标志传输结束，获取高位 bit
    let opcode = byte1 & 0x0f; //截取第一个字节的后 4 位，即 opcode 码
    ...
    let length = byte2 & 0x7f; //获取length属性，也是小于126数据长度的数据真实值
    if (length > 125) {
      if (buf.length < 8) {
    ...
    if (length === 126) {
      ...
      const highBits = buf.readUInt32BE(2); //(1/0)1111111，切记 MSB 最高位是 0
      if (highBits != 0) {
    ...
    // 如果有 mask 标志位，默认都是有的
    if (MASK) {
      const maskBytes = buf.slice(idx, idx + 4); //获取掩码数据
      idx += 4; //指针前移到真实数据段
      payload = buf.slice(idx, idx + length); // 数据长度的单位是字节
  ...
  _doSend(opcode, payload) {
    let len = Buffer.byteLength(payload);
